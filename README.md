...
- Competition: tenders received per lot, the share of competitive lots with a single tender, and the share of lots given as direct awards (negotiated without a prior call for competition). Before eForms (late 2023) tender counts are per procedure, so multi-lot procedures only count when a single tender came in. Staffing platforms that report "1 tender" by default are excluded per year. "Few bidders" flags buyers where at least half the lots drew one tender, 25 points above the peer median.
