...
uv run python etl/enrich.py    # PDOK geocoding + CBS population, cached in data/cache/
...
- `etl/enrich.py`: postcode/municipality/province lookups and population
- `etl/model.sql`: all analysis logic: procedure linking, supplier identity, buyer roll-up, value cleaning, concentration, peer groups, flags
